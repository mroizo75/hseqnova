...
export function AddTrainingRequirementDialog({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!formData.courseKey) {
      toast({
        ...
        description: "Select a course template",
      });
      return;
    }
    ...
    if (result.success) {
      toast({
        ...
        description: "This course is now required for the risk",
      ...
      toast({
        variant: "destructive",
        title: "Could not add",
        description: result.error || "The training requirement could not be added",
      });
    }
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent>
        <DialogHeader>
          <DialogTitle>Add training for this risk</DialogTitle>
          <DialogDescription>
            Required training so people can control the risk (MHSWR 1999 reg.13)
          </DialogDescription>
        </DialogHeader>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="space-y-2">
            <Label htmlFor="reason">Why this course (optional)</Label>
            <Textarea
              id="reason"
              placeholder="e.g. New equipment on this task — MHSWR 1999 reg.13(2)(b)"
